...
def generate_candidate_keywords(sentence_list, stopword_pattern, min_char_length=1, max_words_length=5):
	...
	for idx, s in enumerate(sentence_list):
		...
		phrases = tmp.split("|")
		idxToKeywords.append([])
		for phrase in phrases:
			phrase = phrase.strip().lower()
			if phrase != "" and is_acceptable(phrase, min_char_length, max_words_length):
				phrase_list.append(phrase)
				idxToKeywords[idx].append(phrase)
	return phrase_list, idxToKeywords
...
def is_acceptable(phrase, min_char_length, max_words_length):
	...
	words = phrase.split()
	if len(words) > max_words_length or len(words) < 2:
		return 0
...
class Rake(object):
	...
	def run(self, sentence_list):
		...
		phrase_list, self.idxToKeywords = generate_candidate_keywords(sentence_list, self.__stop_words_pattern, self.__min_char_length, self.__max_words_length)

		word_scores = calculate_word_scores(phrase_list)

		keyword_candidates = generate_candidate_keyword_scores(phrase_list, word_scores, self.__min_keyword_frequency)

		self.keywords = sorted(six.iteritems(keyword_candidates), key=operator.itemgetter(1), reverse=True)
